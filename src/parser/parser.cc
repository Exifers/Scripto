...
Parser::Parser(std::vector<Token> tokens) : i_(0), tokens_(tokens) {}

std::shared_ptr<Node> Parser::parse() { return parse_exps(); }

std::shared_ptr<Exps> Parser::parse_exps() {
...
std::shared_ptr<PrintExp> Parser::parse_print() {
  eat(Token::PRINT);
  return std::make_shared<PrintExp>(parse_value());
}

std::shared_ptr<Exp> Parser::parse_name() {
...
std::shared_ptr<FunctionDec> Parser::parse_function_dec() {
  eat(Token::FUNCTION);
...
std::shared_ptr<RepeatStmt> Parser::parse_repeat() {
  eat(Token::REPEAT);
  eat(Token::LPAR);
  auto num = parse_num();
  eat(Token::RPAR);
...
void Parser::eat(Token::token_t token) {
  if ((size_t)i_ == tokens_.size())
    throw std::invalid_argument(std::string("Unexpected end of file"));
  if (token != tokens_[i_].get_type())
    throw std::invalid_argument(
        std::string("Unexpected token: ") + tokens_[i_].get_value() +
        ", expected token of type: " + std::to_string(token));
  i_++;
}
...
bool Parser::has_next() const { return (size_t)i_ < tokens_.size(); }
