...
Token::Token(token_t type, std::string value) : type_(type), value_(value) {}

const std::string &Token::get_value() const { return value_; }

const Token::token_t &Token::get_type() const { return type_; }
...
std::string Lexer::atohex(const char *input) {
  std::stringstream ss;
...
std::vector<Token> Lexer::scan(const std::string &input) {
...
bool Lexer::match(const std::string &input, const std::string &simple_pattern) {
  regex_t preg;
  int error;
