...
Executer::visit(AssignExp& assignExp) {
  ...
  switch (assignExp.get_rhs()->get_type()) {
      case Value::INT:
        set_int_value(assignExp.get_lhs()->get_name(), assignExp.get_rhs()->get_int_value());
        break;
      case Value::STRING:
        set_string_value(assignExp.get_lhs()->get_name(), assignExp.get_rhs()->get_string_value());
        break;
  }
}
...
Executer::visit(IfStmt& ifStmt) {
  ...
  if (ifStmt.get_lhs()->get_type() != ifStmt.get_rhs()->get_type()) {
    throw std::invalid_argument("Cannot compare values of type " + std::to_string(ifStmt.get_lhs()->get_type()) +
    " and " + std::to_string(ifStmt.get_rhs()->get_type()));
  }

  switch (ifStmt.get_lhs()->get_type()) {
      case Value::INT:
        if (ifStmt.get_lhs()->get_int_value() == ifStmt.get_rhs()->get_int_value()) {
        ...
        break;
      case Value::STRING:
        if (ifStmt.get_lhs()->get_string_value() == ifStmt.get_rhs()->get_string_value()) {
          ifStmt.get_exps()->accept(*this);
        }
        else if (ifStmt.getElse_exps() != nullptr) {
          ifStmt.getElse_exps()->accept(*this);
